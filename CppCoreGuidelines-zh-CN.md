...
* 在虚函数方面，应该推广非虚接口吗？是的。（公开非虚的 `foo()` 调用私有或受保护的 `do_foo()`）？不是新东西，locale 和 I/O 流都在用，不过貌似对它的强调不够。
