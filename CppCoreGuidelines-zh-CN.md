...
所谓强制实施，可以是通过代码评审，通过静态分析，通过编译期，或者通过运行时检查来进行的。
...
* **type**: 消除类型违规（如通过强制转换（cast）、联合（union）或者变参（varargs）把 `T` 重解释为 `U`）
...
编译期是不会去读注释（或设计文档）的，许多程序员也（固执地）不去读它们。
而代码中所表达的东西是带有明确的语义的，并且（原则上）是可以由编译期和其他工具进行检验的。
...
`month` 的第一个声明，显然是要返回一个 `Month`，而且不会修改 `Date` 对象的状态。
...
理想情况下，这个 `Expects(x >= 0)` 应当是 `sqrt()` 的接口的一部分，但轻易做不到这点。当前，我们将之放入定义（函数体）之中。
...
前条件是可以用许多方式来说明的，包括注释，`if` 语句，以及 `assert()`。这些方式使其难于与普通代码之间进行区分，难于进行更新，难于利用工具来操作，而且可能具有错误的语义（你真的总是想要在调试模式中止程序而在生产运行中不做任何检查吗？）
...
【无法强制实施】 要把各种对前套件进行断言的方式都找出来是不可行的。对那些易于识别的（如 `assert()`）实例给出警告的做法，其意义在缺少语言设施的前提下是有问题的。
...
理想情况下，后条件应当在接口或声明中说明，让使用者易于见到它们。
只有那些与使用者有关的后条件才应当在接口中说明。
仅与内部状态相关的后条件应当属于定义或实现。
