...
##### 示例
    ...
    Foo& Foo::operator=(Foo&& a)       // OK，但增加了成本
    {
...
ISO 标准中对标准库容器类仅仅保证了“有效但未指明”的状态。貌似这样做在差不多十年的实验性和产品级代码的使用中并未造成什么问题。入股ONI找到了反例的话，请联系各位编辑。本条规则更多的是提醒小心并强调完全的安全性。
...
* 有任务的基类构造函数，比如吧对象注册到什么地方的时候，可能是需要构造函数的。
...
##### 理由
 ...
 ??? Herb: 这里第二次提到了实现继承。我对此很存疑，即便对单实现继承也是，更别说貌似更吓人的多实现继承了——我觉得即便是基于策略的设计也并不真的需要从策略类型继承。我是不是缺少几个好例子，还是说咱们得考虑把这个作为一种反模式来劝阻？
...
??? Herb: 进行想法的强制实施怎么样：对继承了多个非空基类的任何类型进行标记？
...
### <a name="Renum-set"></a>Enum.2: 采用枚举来表示具名常量的集合
...
### <a name="Renum-unnamed"></a>Enum.6: 采用无名枚举来 ???
...
这部分是因为我们确实需要在基础的资源包装器的实现中一起使用有所有权的“原生指针”和简单的指针。
...
### <a name="Rr-scoped"></a>R.5: 避免不必要的堆分配
...
由 `new` 所返回的指针应当属于一个资源包装器（它将调用 `delete`）。
...
* 【简单】 如果函数接受智能指针类型（重载了 `operator->` 或者 `operator*`，包括 `unique_ptr` 或 `shared_ptr`）的参数，但仅调用了它的 `operator*`，`operator->` 或者 `get()` 的话，就给出警告。
* 如果智能指针类型（即重载了 `operator->` 或 `operator*` 的类型）的参数是可以复制的，但并未从函数体中复制或移动出去，或者若它是可以移动的，但并未从函数体中移动出去或者作为按值传递参数进行移动，同时它未被改动，而且也未被传递给可能做这些事的其他函数的话，就对其进行标记。这些意味着它的所有权语义并未被用到。
  建议代之以 `T*` 或 `T&`。
...
##### 示例
    ...
    void sink(unique_ptr<widget>); // 消费掉了这个 widget

    void sink(widget*);            // 仅仅使用了这个 widget
...
* 【简单】〔基础〕 如果函数以右值引用接受 `Unique_ptr<T>` 参数，则给出警告。建议代之以按值传递。
...
使用 `uninitialized` 或某个哨位值的做法只是一种症状，而并非一种
解决方案：
...
这个貌似无害的宏会让把某个大写的 `C` 替换为小写的 `c` 导致一个严重的控制流错误。
...
不是所有人都知道一年中有 12 个月份，号码是 1 到 12。更好的做法是：
    ...
    constexpr int month_count = 12;   // 月份号码为 1..12

    for (int m = first_month; m <= month_count; ++m)   // 好多了
        cout << month[m] << '\n';
...
请勿在一般的代码中盲目地尝试遵循这些规则，因为要达成低延迟的目标是需要进行一些额外工作的。
...
**例外**: 有一些例子，其代码永远不会再多线程环境中执行。
不过，也有许多例子，“认为”永远不会再多线程程序中执行的代码
...
简而言之，当两个线程并发（未同步）地访问同一个具名对象，且至少一方为写入方（实施某个非 `const` 操作）时，就会出现数据竞争。有关如何正确使用同步来消除数据竞争的更多信息，请求教于一本有关并发的优秀书籍。
...
### <a name="Rconc"></a>除非绝对必要，请勿使用无锁编程
...
资源泄漏通常是不可接受的。RAII（Resource Acquisition Is Initialization，资源获取即初始化）是最简单，最系统化的避免泄漏方案。
...
这样很啰嗦。在更大型的可能带有多个 `throw` 的代码中，显式的释放将变得重复且易错。
    ...
    void f3(int i)   // OK: 通过资源包装来进行资源管理
    {
...
注意即使 `throw` 是在所调用的函数中暗中发生，这也能正常工作：
    ...
    void f4(int i)   // OK: 通过资源包装来进行资源管理
    {
        auto p = make_unique<int[]>(12);
        // ...
        helper(i);   // 可能抛出异常
...
当没有合适的资源包装时，可以使用 [`final_action` 对象](#Re-finally)来表达清理动作。
...
* [T.21: 通过定义概念来定义完整的操作集合](#Rt-complete)
...
* [T.50: 避免在类型的相同命名空间中编写无约束模板](#Rt-unconstrained-adl)

模板定义的规则概览：

* [T.60: 最小化模板的上下文依赖性](#Rt-depend)
* [T.61: 请勿对成员进行过度参数化（恐怖）](#Rt-scary)
* [T.62: 将无依赖的模板成员置于一个非模板基类之中](#Rt-nondependent)
* [T.64: 用特化来提供类模板的其他实现](#Rt-specialization)
* [T.65: 用标签分派来提供函数的其他实现](#Rt-tag-dispatch)
* [T.66: 用基于 `enable_if` 的选择来进行可选的函数定义](#Rt-enable_if)
...
动态有助于静态：提供通用的，便利的，静态绑定的接口，但内部进行动态派发，这样就可以提供统一的对象布局。这样的例子包括如 `std::shared_ptr` 的删除器的类型擦除。（不过[请勿过度使用类型擦除](#Rt-erasure)。）
...
不过，请不要把它当作是稳定的。它的每个新的用例都可能导致不完整概念的改善。
...
### <a name="Rt-unconstrained-adl"></a>T.50: 避免在类型的相同命名空间中编写无约束模板

##### 理由

即便你觉得不会，ADL 还是会找到这个模板。
...
这条规则可能是不必要的；标准委员会还无法在如何修正 ADL 上达成一致，不过至少让它不再考虑无约束模板可以解决许多的实际问题，并可以消除对这条规则的需求。

##### 强制实施

??? 不幸的是这可能带来许多误报；标准库大量地违反了这条规则，它把许多无约束模板和类型都放到一个命名空间 `std` 之中了。
...
让模板仅对其参数进行操作是一种把依赖减到最少的方式，但通常这是很难做到的。比如说，一个算法通常会使用其他的算法。
...
以仅提供预计之内的灵活性，并避免意外的外部造成的改变。

当你打算为依赖于某个模板类型参数的值 `t` 调用自己的辅助函数 `helper(t)` 时，请将函数放入一个 `::detail` 命名空间中，并把调用限定为 `detail::helper(t);`。否则的话，调用就会变成一个定制点，它可能会替代为调用处于 `t` 的类型所在命名空间中的任何 `helper` 函数——成为了下面的第二种情况，并导致诸如[意外地调用了刚好处于 `t` 的类型的相同命名空间中的同名无约束函数模板](#Rt-unconstrained-adl)这样的问题。

主要有三种方法使调用代码对模板进行定制化。

* 调用成员函数。调用方可以为之提供任何带有这个名字的成员函数的类型。
...
* 无限定地调用非成员函数。调用方可以为之提供任何类型，并在调用方的上下文或者类型的命名空间中提供一个这样的函数。

        template<class T>
        void test(T t)
        {
            f(t);     // 要求 f(/*T*/) 出现于调用方的作用域或者 T 的命名空间中
        }

* 调用一个“特征”——通常是用以计算某个类型的类型别名，或者是用以计算某个值的 `constexpr` 函数，或者少数情况下为对用户的类型进行了特化的传统的特征模板。

        template<class T>
        void test(T t)
        {
            test_traits<T>::f(t);    // 要求定制化 test_traits<> 以获得非预置的函数和类型
...
使一个带有许多函数，尤其是有许多虚函数的类层次进行模板化，会导致代码爆炸。
...
许多情况下都可以通过不为基类进行参数化而提供一个稳定的接口；参见[这条规则](#Rt-abi)。
...
* [CPL.3: 当一定要用 C 来作为接口时，应在使用这些接口的代码中使用 C++](#Rcpl-interface)
...
* [SF.6: `using` 指令，可以为迁移而使用，可以为基础程序库使用（比如 `std`），或者在局部作用域中使用](#Rs-using)
* [SF.7: 请勿在头文件中使用 `using` 指令](#Rs-using-directive)
...
例子包括 `.hh` 和 `.cxx` 等。请以类似方式使用这些名字。本文档中我们把 `.h` 和 `.cpp` 作为头文件和实现文件的简便提法，虽然实际上的扩展名可能是不同的。
...
### <a name="Rs-using"></a>SF.6: `using` 指令，可以为迁移而使用，可以为基础程序库使用（比如 `std`），或者在局部作用域中使用
...
### <a name="Rs-using-directive"></a>SF.7: 请勿在头文件中使用 `using` 指令

##### 理由

这样做使 `#include` 一方无法有效地进行区分并使用其他方式。
...
* [SL.10: 优先采用 STL 的 `array` 或 `vector` 而不是 C 数组](#Rsl-arrays)
...
### <a name="Rsl-vector"></a>SL.11: 除非有理由使用别的容器，否则默认情况应优先采用 STL 的 `vector`

##### 理由

`vector` 和 `array` 是仅有的能够提供最快的通用访问（随访问，同时对于向量化友好），最快的默认访问模式（从头到尾或从尾到头方式是对预读器友好的），以及最少的空间耗费（连续布局没有每个元素的开销，而且是 cache 友好的）的标准容器。通常你都需要对容器进行元素的添加和删除，因此默认应当采用 `vector`；如果并不需要改动容器的大小的话，则应采用 `array`。
...
### <a name="Ra-stable"></a>A.1 将代码中的稳定部分和不稳定的部分进行区分

???

### <a name="Ra-reuse"></a>A.2 将潜在可复用的部分作为程序库

???

### <a name="Ra-lib"></a>A.3 将潜在需要分别维护的部分作为程序库

???

# <a name="S-not"></a>伪规则和错误的看法

本部分包含一些在不少地方流行的规则和指导方针，但是我们慎重地建议不要采纳它们。
在我们所推荐并以各项指导方针所支持的编程风格的情况中，这些伪规则是有害的。
...
* [RF.C++: C++ 编程 (C++11/C++14)](#SS-Cplusplus)
* [RF.web: 网站](#SS-web)
...
编写成在这样的语言剖面配置下仍免于警告的代码，可以认为是遵循这个剖面配置的。而遵从的代码则可以认为对于该剖面配置的目标安全属性来说是安全的。遵从的代码不会成为这种性质的错误的根本原因，虽然程序中可能从其他的代码引入这样的错误。一个剖面配置还可能会引入一些额外的库类型，以简化遵从性并鼓励编写正确的代码。
...
### <a name="Pro-type-varargs"></a>Type.8: 避免从 vararg 中读取或传递 vararg 参数。优先代之以变参模板的参数
...
##### 示例
    ...
    void stable_sort(Sortable& c)
        // 对 c 根据由 < 决定的顺序进行排序，保持相等元素（由 == 定义）的原始相对顺序
    {
        // ... 相当多的不平常的代码行 ...
    }
...
**原理**: 当名字反映类型而不是功能时，它将变得难于为提供其功能而改变其所使用的类型。
带有类型编码的名字要么啰嗦要么难懂。
匈牙利记法（至少在强静态类型语言中）是有害。
...
### <a name="Rl-all-caps"></a>NL 9: 将 `ALL_CAPS` 仅用于宏的名字
...
对你自定义的类型的名字进行首字母大写，以将其与标准库类型向区分。
...
# <a name="S-modernizing"></a>附录 B: 代码的现代化
...
许多情况下，都需要某种自动进行“现代化”的工具支持来进行主要的升级工作。

对代码现代化的目的在于简化新功能的添加，简化维护工作，以及增加性能（吞吐量或延迟），和更好地利用当代的硬件能力。
...
如果有分析工具（即便是代码变换工具）的支持的话，代码现代化可以变得更快，更简单，而且更安全。
...
以下是对最后一种选项的一个例子：
    ...
    protected:
        B() { /* ... */ }
        virtual void PostInitialize()    // 构造后立刻调用
            { /* ... */ f(); /* ... */ }   // 好: 进行虚派发是安全的
    public:
        virtual void f() = 0;
...
* [绝不在持有未被包装器所拥有的资源时抛出异常](#Cr-never)
* [“原生”的指针或引用不可能是资源包装器](#Cr-raw)
* [绝不让指针的生存期超过其所指向的对象](#Cr-outlive)
* [用模板来表现容器（和其他资源包装器）](#Cr-templates)
* [按值返回容器（依靠移动或复制消除来获得性能）](#Cr-value-return)
* [若类为资源包装器，则它需要构造函数，析构函数，复制以及移动操作](#Cr-handle)
* [若类为容器，则为其提供一个初始化式列表构造函数](#Cr-list)
...
这个类是一个资源包装器。它管理各个 `T` 对象的生存期。为此，`Vector` 必然要对[一组特殊操作](???)（几个构造函数，析构函数，等等）进行定义或弃置。
...
防止泄漏的基本技巧是让所有的资源都被某种带有回档析构函数的资源包装器所拥有。检查工具能够查找出“裸 `new`”。给定一组 C 风格的分配函数（如 `fopen()`），检查工具也能够查找出未被资源包装器管理的使用点。一般来说，可以带着怀疑看待“裸指针”，对其进行标记和分析。如果没有人为输入的话，时无法产生资源的完整列表的（“资源”的定义有些过于宽泛），不过可以用一个资源列表来对工具进行“参数化”。

### <a name="Cr-never"></a>绝不在持有未被包装器所拥有的资源时抛出异常
...
当 `i == 0` 时 `a file` 的文件句柄就会泄漏。另一方面，`another file` 的 `ifstream` 则将会（在销毁时）正确关闭它的文件。如果你必须显式使用指针而不是带有特定语义的资源包装器的话，可以使用带有自定义删除器的 `unique_ptr` 或 `shared_ptr`：
...
`span` 和 `string_span` 的使用能够提供巨大的帮助（它们并非资源包装器）。

### <a name="Cr-raw"></a>“原生”的指针或引用不可能是资源包装器
...
大多数编译器已经能对简单情况进行警告，而且它们带有可以更进一步的信息。将函数所返回的任何指针都当作是可疑的。用容器、资源包装器和视图（例如 `span`，它不是资源包装器）来减少需要检查的情形。上手时，可将带有析构函数的类都当作是资源包装器处理。

### <a name="Cr-templates"></a>用模板来表现容器（和其他资源包装器）
...
简化代码并消除一种进行显式内存管理的需要。将对象递交给外围作用域，由此扩展其生存期。参见 [F.20，有关“输出（Out）”值的一般条款](#Rf-out)。
...
检查函数所返回额指针和引用，看看它们是否被赋值给资源包装器（如 `unique_ptr`）。


### <a name="Cr-handle"></a>若类为资源包装器，则它需要构造函数，析构函数，复制以及移动操作
...
一般来说，工具是无法知道类是否是资源包装器的。不过，如果类带有某种[默认操作](#SS-ctor)的话, 它就得拥有全部，而如果类中有成员为资源包装器的话，它也应被当做是资源包装器。
