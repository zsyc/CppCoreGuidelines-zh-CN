...
* [构造函数](#SS-ctor)
* [派生 `class`](#SS-hier)
* 析构函数：[以及构造函数](#Rc-matched)，[何时需要？](#Rc-dtor)，[不可失败](#Rc-dtor-fail)
* [异常](#S-errors)
* `for`：[范围式 `for` 和 `for`](#Res-for-range), [`for` 和 `while`](#Res-for-while) [`for`-初始化式](#Res-for-init)，[空循环体](#Res-empty)，[循环变量](#Res-loop-counter)，[循环变量的类型 ???](#Res-???)
* [`inline`](#S-class)
* [初始化](#S-???)
...
* [`static_assert`](#S-???)
* `struct`：[用于组织数据](#Rc-org)，[没有不变式时使用](#Rc-struct)，[不能有私有成员](#Rc-class)
* [`template`](#S-???)
* [`unsigned`](#S-???)
* [`virtual`](#SS-hier)
...
关于何时从一个工厂中返回 `shared_ptr`，另见 [C.50](#Rc-factory)。
...
* [Sl.str.11: 当需要改动字符串时，使用 `gsl::string_span` 而不是 `std::view`](#Rstr-span)
...
我们所知的只不过是它可能是 nullptr 或者指向至少一个字符
    ...
    void f1(czstring s);    // s 是 C 风格字符串而不是 nullptr
...
???
### <a name="Rstr-span"></a>Sl.str.11: 当需要改动字符串时，使用 `gsl::string_span` 而不是 `std::view`
...
* 经选择的未定义行为：从 Gaby Dos Reis 为 WG21 研究小组开发的 UB 列表入手
* 经选择的未指明行为：处理可移植性问题。
...
参见 /docs 文件夹中的初始设计。正式的规则仍在指定中（2017 年 3 月）。
